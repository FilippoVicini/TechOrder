...
if (isset($_POST['new_ingredient'])){
    ...
    $query = "INSERT INTO `magazzino`(`ID`, `nome`, `quantità`, `restaurant_id`, `date_added`) VALUES ('','".$ingredient_name."','0','".$restaurant_id."','".date("Y-m-d H:i:s")."')";
    mysqli_query($conn, $query);
}

if (isset($_POST['INCREASE']) || isset($_POST['DECREASE'])){
    $val = isset($_POST['INCREASE']) ? "+50" : "-50";
    $id = isset($_POST['INCREASE']) ? $_POST['INCREASE'] : $_POST['DECREASE'];
    
    $query = "UPDATE `magazzino` SET `quantità`= `quantità`".$val." WHERE `ID` = ".$id;
    mysqli_query($conn, $query);
}
...
<html lang="en"><script type="text/javascript">window.loop11ExtTerritory = true;</script><head>
    ...
    <title>Rifornimento mf-shores</title>
  ...
  <div class="sidebar close">
   ...
   <i class="bi bi-globe" style="margin-top:8%"></i>
    ...
    <ul class="nav-links">
      ...
      <li>
        <a href="#">
          ...
          <span class="link_name">Rifornimento</span>
        </a>
        <ul class="sub-menu blank">
    ...
    <div class="profile-details">
      ...
      <i href="http://mf-shores.it/login/" class="bx bx-log-out" ></i>
    </div>
  </li>
</ul>
  </div>
...
<center>
        ...
        <div class="MF-DigitalWaiter_title_panel">
            <h1 class="MF-DigitalWaiter_title" style="font-family: Montserrat !important;">Rifornimenti</h1>
        </div>
...
while ($ingredient = mysqli_fetch_assoc($ingredients)){
    ...
    <div class=\"piccola_tavoli_fighi\" style=\"display: inline-block; padding-bottom:10px; width:33%;background: white;\">
        ...
        <form action=\"./index.php\" method=\"POST\" style=\"display: inline-block\">
            <button class=\"btn btn-success\" name=\"INCREASE\" value=\"".$id."\">+</button>
        </form>
        
        
         <input style=\"width:23%;\" class=\"form-control\" onchange=\"myFunction()\" type=\"number\"  min=\"0\"style=\"display: inline; width: \"7.ch\" onkeypress=\"this.style.width = (this.value.length + 1) + 'ch';\" value=\"".$quantity."\">g</input>
         
        
        <form action=\"./index.php\" method=\"POST\" style=\"display: inline-block\">
            <button class=\"btn btn-danger\" name=\"DECREASE\" value=\"".$id."\">-</button>
        </form> 
...
<form action="index.php" method="POST">
 <div class="MF-DigitalWaiter_title_panel">
    ...
    <center class="d-flex justify-content-center" style="margin-left:25%; margin-right: 25%; width: 50%;"><div class="piccola_aggcategoria" style="display: inline-block; padding-bottom:10px;">


<br><input type="text" maxlength="75" id="new_ingredient" name="new_ingredient" style="background-color: #f2f2f2; border:3px; width:80%; height:50px; font-size: 16px; outline:none; margin-top:2%; margin-bottom:2%; border: 2px solid #ababab; border-radius:10px;" class="px-3" placeholder="Nome ingrediente" required>



<br><button id="add_" type="submit" class="btn btn-success btn-lg px-5 border border-4 border border-success fs-4 font-monserrat fw-bold mx-3 px-4" style="float:center; margin-top:2%; height:50px;">AGGIUNGI</button>
